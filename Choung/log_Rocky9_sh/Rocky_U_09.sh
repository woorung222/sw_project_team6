...
# run_cmd 적용
PRIMARY_GIDS=$(run_cmd "[U_09_1] 사용 중인 Primary GID 목록 추출" "cut -d: -f4 /etc/passwd | sort -u")
...
while IFS=: read -r G_NAME G_PASS G_GID G_MEMBERS; do
    ...
    if [ "$G_GID" -ge 1000 ]; then
        ...
        if [ -z "$G_MEMBERS" ]; then
            ...
            # (반복문 내부는 run_cmd 자제 -> 로그 폭주 방지, 로직 검증용 grep만 실행)
...
if [ $U_09_1 -eq 1 ]; then
    log_basis "[U_09_1] 구성원이 없는 GID(1000이상) 그룹 발견: $VULN_GROUPS" "취약"
else
    log_basis "[U_09_1] 구성원이 없는 불필요한 그룹이 존재하지 않음" "양호"
fi
