...
# grep 명령어를 run_cmd로 감싸서 실행 로그 기록
ROOT_GROUP_LINE=$(run_cmd "[U_08_1] root 그룹 정보 조회" "grep '^root:' /etc/group")

# 4번째 필드(그룹원 목록) 추출 (로직 유지)
...
if [ -z "$GROUP_MEMBERS" ]; then
    ...
    U_08_1=0
    log_basis "[U_08_1] root 그룹 멤버가 비어있음 (Primary Group 사용 중)" "양호"
elif [ "$GROUP_MEMBERS" == "root" ]; then
    # 2. 그룹원에 'root'만 명시되어 있는 경우 -> 양호
    U_08_1=0
    log_basis "[U_08_1] root 그룹에 root 계정만 존재함" "양호"
else
    # 3. 그 외 다른 계정이 포함된 경우 -> 취약
    # 예: "root,user1" 또는 "user1"
    U_08_1=1
    log_basis "[U_08_1] root 그룹에 불필요한 계정이 포함됨: $GROUP_MEMBERS" "취약"
fi
