...
# 자동 조치 가능 유무 : 가능
# 플래그 설명:
#   U_38_1 : [systemd] echo, discard, daytime, chargen 서비스 활성화 발견
#   U_38_2 : [xinetd] xinetd 설정 내 해당 서비스 활성화 발견
#   U_38_3 : [inetd] inetd 설정 내 해당 서비스 활성화 발견
...
# 점검 대상 서비스 정규식 (가이드라인 p.85 기준)
DOS_SVCS="echo|discard|daytime|chargen"

# 1. [systemd] 점검 (U_38_1) - PDF p.84 
# 서비스 유닛 또는 소켓이 'active' 상태인지 확인
# 패키지가 미설치되어 있으면 아무것도 출력되지 않으므로 안전함
...
if [[ -d "/etc/xinetd.d" ]]; then
    # 해당 서비스 파일이 존재하고, disable = no 설정이 있는지 확인
...
# 3. [inetd] 점검 (U_38_3) - PDF p.83 
if [[ -f "/etc/inetd.conf" ]]; then
    # 주석(#)을 제외한 행에서 서비스 설정 존재 여부 확인
...
if [[ $VULN_STATUS -eq 0 ]]; then
    echo -e "${GREEN}[양호]${NC} DoS 공격에 취약한 모든 서비스(echo/discard/daytime/chargen)가 비활성화되어 있습니다."
else
    echo -e "결과: ${RED}[취약]${NC}"
fi
