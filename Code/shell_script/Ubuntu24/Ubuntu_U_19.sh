...
####### 자동 조치 가능 유무 : 
####### 자동 조치 불가능 사유 : 
####### [취약 조건] : /etc/hosts 파일의 소유자가 root가 아니거나, 권한이 644 이하가 아닌 경우
...
if [ -f /etc/hosts ]; then
		OWNER_NAME=`ls -l /etc/hosts | awk '{print $3}'`
		if [[ $OWNER_NAME =~ root ]]; then
			HOSTS_PERMISSION=`stat /etc/hosts | grep -i 'Uid' | awk '{print $2}' | awk -F / '{print substr($1,3,3)}'`
			if [ $HOSTS_PERMISSION -le 600 ]; then
				OWNER_PERMISSION=`stat /etc/hosts | grep -i 'Uid' | awk '{print $2}' | awk -F / '{print substr($1,3,1)}'`
				if [ $OWNER_PERMISSION -eq 0 ] || [ $OWNER_PERMISSION -eq 2 ] || [ $OWNER_PERMISSION -eq 4 ] || [ $OWNER_PERMISSION -eq 6 ]; then
					GROUP_PERMISSION=`stat /etc/hosts | grep -i 'Uid' | awk '{print $2}' | awk -F / '{print substr($1,4,1)}'`
					if [ $GROUP_PERMISSION -eq 0 ]; then
						OTHER_PERMISSION=`stat /etc/hosts | grep -i 'Uid' | awk '{print $2}' | awk -F / '{print substr($1,5,1)}'`
		...
		#echo "※ U-19 결과 : N/A" >> $resultfile 2>&1
